...
S_DESC  struc                                   ;Структура сегментного дескриптора
    LIMIT       dw 0                            ;Лимит сегмента (15:0)    
...
PREPARE_RTC:                                    ;Подготовка часов RTC 
    mov al, 0Bh                                 ;Выбрать регистр состояния 0Bh
    out 70h, al                                 ;Записать значение регистра 0Bh в 70 порт
    in  al, 71h                                 ;Прочитать значение регистра 0Bh из 71 порта
    or  al, 00000100b                           ;Установить второй бит в 1 - формат представления время в двоичном виде
    out 71h, al                                 ;Записать измененное значение
    
ENABLE_A20:                                     ;Открыть линию A20
    in  al, 92h                                                                              
    or  al, 02h                                 ;Установить бит 1 в 1                                                   
    out 92h, al                                                            

SAVE_MASK:                                      ;Сохранить маски прерываний     
    in      al, 21h
    mov     INT_MASK_MASTER, al                 ;Маска ведущего контроллера
    in      al, 0A1h
    mov     INT_MASK_SLAVE, al                  ;Маска ведомого контроллера
...
LOAD_GDT:                                       ;Заполнить глобальную таблицу дескрипторов            
    ...
    xor dh, dh
    shl ax, 4                                   ;Побитово сдвигаем влево
    shr dx, 4                                   ;Побитово сдвигаем вправо
    mov si, ax
    mov di, dx
...
WRITE_GDT:                                      ;Заполнить дескриптор GDT
    ...
    mov ax, si
    mov dx, di
    add ax, offset GDT                          ;Складываем со смещением
    adc dx, 0
    mov [bx][S_DESC.BASE_L], ax                 ;Записываем физический адрес GDT в элемент GDT, описывающий саму GDT
...
REINITIALISE_CONTROLLER:                        ;Переиницализация контроллера прерываний               
    ...
    out 20h, al                                 ;Переинициализируем ведущий контроллер
    out 0A0h, al                                ;Переинициализируем ведомый контроллер
    mov al, 08h                                 ;ICW2 - номер базового вектора прерываний
    out 21h, al                                 ;ведущего контроллера
...
SHOW_TIMER:
    mov  al, 00h                                ;Выбрать регистр секунд cmos
    out  70h, al
...
DATA_BEGIN      = $
    ...
    KEY_SCAN_CODE       db  1 dup(?)            ;Скан-код последней нажатой клавиши
